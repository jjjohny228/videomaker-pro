...
db = pw.SqliteDatabase('video_editor.db')
...
class AutoIntroSetting(_BaseModel):
    ...
    """
    brand_kit = pw.ForeignKeyField('BrandKit', backref='auto_intro_setting_ref', unique=True, on_delete='CASCADE',
    ...
    typewriter_speed = pw.IntegerField(default=50, constraints=[pw.Check('typewriter_speed >= 1 AND typewriter_speed <= 1000')],
                                       help_text="Speed of the typewriter effect (e.g., characters per second).")
...
class Caption(_BaseModel):
    ...
    Intended for a One-to-One relationship with BrandKit.
    """
    brand_kit = pw.ForeignKeyField('BrandKit', backref='caption_specification', unique=True, on_delete='CASCADE',
...
class Voice(_BaseModel):
    ...
    language = pw.CharField(null=True, blank=True, help_text="Language code (e.g., en-US, ru-RU).")
...
class BrandKit(_BaseModel):
    ...
    intro_clip_path = pw.CharField(null=True,
                                   help_text="Path to the intro video file (recommended length 3-7 seconds).")
    randomize_clips = pw.BooleanField(default=False,
                                      help_text="If checked, clips from the kit will be used in random order.")

    watermark_path = pw.CharField(null=True, help_text="Path to a 16:9 PNG watermark image with transparency.")
    ...
    avatar_clip_path = pw.CharField(null=True, help_text="Path to an animated looped avatar clip (e.g., talking head).")
    ...
    voice = pw.ForeignKeyField(Voice, backref='used_by_brand_kits', null=True, on_delete=pw.SET_NULL,
